from map import Map
from entity import Tank, Soldier, Helicopter, FighterJet, Artillery, InfantryFightingVehicle, UAV, ArmoredVehicles, MissileVehicle, Vehicles, Airplane
...
def generate_entities(scale, advantage, current_time, prev_entities_data=None, location_histories=None):
    ...
    entities_data = []
    entity_types = [Tank, Soldier, Helicopter, FighterJet, Artillery, InfantryFightingVehicle, UAV, ArmoredVehicles, MissileVehicle, Vehicles, Airplane]  # 实体类型
    ...
    # 设置中间缓冲区（不生成实体）
    buffer_ratio = 0.1  # 中间区域占地图比例
    offset_range = 0.01  # 偏移值范围，用于微调实体位置
    ...
    if prev_entities_data is None:
        ...
        for i in range(num_entities):
            ...
            # 生成实体         
            entity_type = random.choices(entity_types, k=1, weights=[3, 1, 2, 3, 2, 2, 3, 3, 3, 3, 2])[0]
